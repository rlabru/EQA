...
{ // CY fix
    if (_cursorPosition < (_bPosFirst * 2))
        verticalScrollBar()->setValue((int)(_cursorPosition / 2 / BYTES_PER_LINE) - (_rowsShown/2) );
    if (_cursorPosition > ((_bPosFirst + (_rowsShown - 1)*BYTES_PER_LINE) * 2))
        verticalScrollBar()->setValue((int)(_cursorPosition / 2 / BYTES_PER_LINE) + (_rowsShown/2) - _rowsShown ); /* - _rowsShown + 1*/
    viewport()->update();
}
...
void QHexEdit::markBytes(qint64 begin, qint64 count)
{ // CY add
    qint64 curPos = begin*2;
    setCursorPosition(curPos + count*2);
    resetSelection(curPos);
    setSelection(curPos + count*2);
    ensureVisible();
}
